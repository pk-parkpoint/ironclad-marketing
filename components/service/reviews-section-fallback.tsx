import Link from "next/link";
import { REVIEWS } from "@/content/reviews";

const STATS = [
  { value: "26", label: "5-STAR REVIEWS" },
  { value: "131", label: "COMPLETED JOBS" },
  { value: "97", label: "HAPPY CUSTOMERS" },
] as const;

function GoogleGIcon({ size = 24 }: { size?: number }) {
  return (
    <svg aria-hidden="true" fill="none" height={size} viewBox="0 0 24 24" width={size}>
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1Z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23Z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18A10.96 10.96 0 0 0 1 12c0 1.77.42 3.45 1.18 4.93l3.66-2.84Z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53Z"
        fill="#EA4335"
      />
    </svg>
  );
}

function StarRow() {
  return (
    <div aria-label="5 out of 5 stars" className="flex items-center gap-0.5" role="img">
      {Array.from({ length: 5 }).map((_, index) => (
        <svg
          aria-hidden="true"
          fill="#FBBC04"
          height="16"
          key={index}
          stroke="#FBBC04"
          strokeWidth="1.5"
          viewBox="0 0 24 24"
          width="16"
        >
          <path d="m12 2.5 2.9 6 6.6 1-4.8 4.7 1.2 6.6L12 17.8 6.1 20.8l1.2-6.6L2.5 9.5l6.6-1L12 2.5Z" />
        </svg>
      ))}
    </div>
  );
}

export function ReviewsSectionFallback() {
  const featuredReviews = REVIEWS.slice(0, 3);

  return (
    <section
      aria-label="Customer reviews and stats"
      className="bg-white home-deferred-section"
      style={{ contentVisibility: "auto", containIntrinsicSize: "1040px" }}
    >
      <div className="mx-auto w-full max-w-[1280px] px-4 pt-16 md:px-6">
        <div className="mx-auto grid max-w-[960px] gap-8 py-12 md:grid-cols-3">
          {STATS.map((stat) => (
            <div className="flex flex-col items-center px-4 py-2" key={stat.label}>
              <span className="text-4xl font-bold text-[#2563EB] md:text-[48px]">{stat.value}</span>
              <span className="mt-2 text-[13px] font-semibold uppercase tracking-[0.1em] text-[#6B7280]">
                {stat.label}
              </span>
            </div>
          ))}
        </div>

        <h2 className="mt-12 mb-2 text-center text-[32px] font-bold leading-[1.2] text-[#2563EB]">
          What Austin Homeowners Are Saying
        </h2>
        <div className="mx-auto mb-10 h-[3px] w-12 rounded-sm bg-[#2563EB]" />

        <div className="grid gap-6 md:grid-cols-3">
          {featuredReviews.map((review) => (
            <article
              className="flex h-full flex-col rounded-lg border border-[#E5E7EB] bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.06)]"
              key={review.id}
            >
              <div className="flex items-center gap-3">
                <div
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-base font-semibold text-white"
                  style={{ backgroundColor: review.avatarColor }}
                >
                  {review.initial}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-[15px] font-semibold text-[#111827]">{review.name}</p>
                  <p className="text-[13px] text-[#9CA3AF]">{review.timeAgo}</p>
                </div>
                <GoogleGIcon />
              </div>

              <div className="mt-4 mb-3">
                <StarRow />
              </div>

              <p className="text-sm leading-relaxed text-[#374151]">{review.text}</p>
            </article>
          ))}
        </div>

        <div className="flex flex-col items-center">
          <Link
            className="mt-10 inline-flex items-center rounded-md bg-[#2563EB] px-8 py-3.5 text-sm font-semibold text-white transition-colors hover:bg-[#1D4ED8] hover:no-underline"
            href="/reviews"
          >
            Read All Reviews &rarr;
          </Link>
          <div className="mt-4 mb-12 inline-flex items-center gap-1.5">
            <GoogleGIcon size={20} />
            <StarRow />
            <span className="text-xs font-semibold uppercase tracking-[0.05em] text-[#6B7280]">4.9/5 RATING</span>
          </div>
        </div>
      </div>
    </section>
  );
}
